...
export class ProductService {
  ...
  private url = 'http://localhost:8081/api/administrative/product';
  ...
  findByFilter(productFilter: ProductFilterDTO): Observable<Product[]> {
    return this.httpClient.post<Product[]>(this.url.concat('/find-by-filter'), JSON.stringify(ProductFilterDTO))
      .pipe(
        catchError(this.handlerError)
      );
  }
